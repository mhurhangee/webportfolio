...
export function ColourfulText({ segments }: { segments: ColoredTextSegment[] }) {
      ...
      {segments.map((segment, segmentIndex) => (
        <React.Fragment key={`segment-${segmentIndex}`}>
          {segment.text.split('').map((char, charIndex) => {
            const globalIndex =
              segments.slice(0, segmentIndex).reduce((acc, curr) => acc + curr.text.length, 0) +
              charIndex;
...
export function LegacyColourfulText({ text }: { text: string }) {
  const colors = React.useMemo(
    () => ['rgb(239 68 68)', 'rgb(234 179 8)', 'rgb(34 197 94)', 'rgb(59 130 246)'],
    []
  );
